...
def generate_article(topic: str) -> Dict[str, str]:
    ...
    # Kimi 可能返回多轮（先调用搜索工具，再生成文章），取最终文本
    raw = ""
    ...
    # 如果模型返回了工具调用但还没有最终文本，需要把工具结果回传再请求一次
    # moonshot-v1-auto 通常会在同一次响应里完成搜索+生成，但做好兜底
    if not raw and resp.choices[0].message.tool_calls:
        log.info("Kimi 返回工具调用，回传结果继续生成...")
        messages = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": prompt},
            resp.choices[0].message,  # assistant 的工具调用消息
        ]
        for tc in resp.choices[0].message.tool_calls:
            messages.append({
                "role": "tool",
                "tool_call_id": tc.id,
                "content": "",  # Kimi 内置工具结果由平台填充，留空即可
            })
