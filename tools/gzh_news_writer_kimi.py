...
def generate_article(topic: str) -> Dict[str, str]:
    ...
    # 如果直接返回了文章内容（无需搜索），直接用
    if choice.message.content:
        raw = choice.message.content.strip()
    else:
        # 第二轮：把搜索结果回传，让模型生成文章
        # 关键：手动构造 messages，不复用 message 对象（避免 reasoning_content 问题）
        ...
        # 为每个 tool call 添加 tool result
        for tc in tool_calls:
            messages.append({
                "role": "tool",
                "tool_call_id": tc.id,
                "content": tc.function.arguments,  # 搜索结果就在 arguments 里
            })
